...
/*
 * Aquí se usan Vertex Arrays con una dificultad añadida. Hay que dimensionar
...
int crear_grada_frontal (config_t c)
{
        ...
        for (i = 0; i < (gdet + 1) * 3; i += 3) {
                ...
                *(escalon+i*3+Z)     = 0.0;
                *(escalon+(i+1)*3+Z) = *(escalon+(i+2)*3+Z)
                                     = paso_v;
        }
...
int crear_grada_lateral (config_t c)
{
        ...
        float r;
        float radio_ex   = valor_decimal(c, c_l_radio)
                           - valor_decimal(c, gs_sep);
        ...
        for (i = gap; i < (gdet - gap); ++i) {
                ...
                for (j = 0; j < gesc; ++j) {
                        ...
                        glTexCoord2f(ss, 0.0);
                        glVertex3f  (r*sin_angulo, r*cos_angulo,
                                     paso_v * (j+1));
